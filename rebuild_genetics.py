...
content = """export type Allele = string;
...
export const LOCI: Record<string, LocusInfo> = {
  ...
  W: {
    name: 'White Markings',
    description: 'Controls dominant white patterns. Some combinations are lethal.',
...
export interface Fox {
  ...
  history?: { year: number; season: string; event: string; type: 'show' | 'breeding' | 'life' }[];
}
...
export function createFoundationalFox(random: () => number = Math.random, gender?: "Dog" | "Vixen"): Fox {
  ...
  return createFox({ genotype, coi: 0, pedigreeAnalyzed: true, gender }, safeRandom);
}
...
export function createFoundationFoxCollection(random: () => number = Math.random): Fox[] {
  ...
  const foxes: Fox[] = [];
  foxes.push(createFoundationalFoxWithGenotype({ A: ['A', 'A'], B: ['B', 'B'] }, safeRandom)); // RED
  foxes.push(createFoundationalFoxWithGenotype({ A: ['A', 'A'], B: ['B', 'b'] }, safeRandom)); // GOLD
  ...
  for (let i = 0; i < 3; i++) {
    const all = [{ A: ['A', 'A'], B: ['B', 'B'] }, { A: ['A', 'A'], B: ['B', 'b'] }, ...crossGenotypes, ...otherGenotypes];
    const genotype = all[Math.floor(safeRandom() * all.length)];
    foxes.push(createFoundationalFoxWithGenotype(genotype, safeRandom));
  }
  const blackFoxIndices = foxes.map((fox, index) => ({ fox, index })).filter(({ fox }) => fox.silverIntensity === 1);
...
export function getRandomEyeColor(phenotypeName: string): string {
  ...
  const weightedColors = possibleColors.map(color => ({ color, weight: EYE_COLOR_WEIGHTS[color] || 10 }));
...
export function getValidEyeColors(genotype: Genotype, silverIntensity: number = 3): string[] {
  ...
  if (W.some(a => a !== 'w') && !L.some(a => a === 'l')) return [...baseColors, "Blue", "Blue Heterochromia"];
...
export function getEyeColorHex(eyeColor: string, baseColor?: string): string {
  const colorMap: Record<string, string> = {
    'Brown': '#8B4513', 'Light Brown': '#D2691E', 'Amber': '#FFBF00', 'Green': '#228B22', 'Grey': '#808080', 'Blue': '#4169E1', 'Red': '#DC143C',
  };
  ...
  if (eyeColor === 'Blue Heterochromia') return 'linear-gradient(45deg, #4169E1 50%, #8B4513 50%)';
