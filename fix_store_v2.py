...
# 1. Ensure GameState interface is correct
game_state_props = """  adminSetCurrency: (updates: { gold?: number; gems?: number }) => void;
...
# Find the messed up adminSetCurrency in GameState
content = re.sub(
    r'adminSetCurrency: \(updates: \{ gold\?: number; gems\?: number \}\) => void;.*?lockForumPost: \(postId: string\) => void;',
    game_state_props,
    content,
    flags=re.DOTALL
)

# 2. Fix useGameStore implementation (the main part)
# We need to ensure all the new actions and state are present in the store object.
# I'll locate showVisibilityMode and insert after it if missing.
...
if 'setCurrentMemberId: (id)' not in content:
    content = re.sub(
...
# 3. Ensure initializeGame is present and correct
initialize_game_body = """
      initializeGame: () => {
        const { foxes } = get();
...
# 4. Ensure resetGame is present and correct
reset_game_body = """
      resetGame: () => {
