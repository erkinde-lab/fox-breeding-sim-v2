...
export function calculatePointsAndMajors(place: number, entries: number): { points: number, isMajor: boolean } {
  let points = 0;
  let isMajor = false;
  ...
  if (entries < 5) {
    ...
    if (place === 1) { points = 3; isMajor = true; }
    if (place === 2) points = 2;
    if (place === 3) points = 1;
  } else if (entries < 20) {
    if (place === 1) { points = 5; isMajor = true; }
    if (place === 2) points = 3;
    if (place === 3) points = 2;
    if (place === 4) points = 1;
    if (place === 5) points = 1;
  } else {
    if (place === 1) { points = 7; isMajor = true; }
    if (place === 2) points = 5;
    if (place === 3) points = 3;
    if (place === 4) points = 2;
    if (place === 5) points = 1;
...
export function runShow(level: ShowLevel, foxes: Fox[], year: number, season: string, hasGroomer: boolean = false, hasTrainer: boolean = false, hasVeterinarian: boolean = false): ShowReport {
  ...
  classes.forEach(cls => {
    ...
    const numPlacings = finalEligible.length >= 20 ? 7 : (finalEligible.length >= 10 ? 5 : 3);
    scored.slice(0, numPlacings).forEach((s, idx) => {
      const place = idx + 1;
      const { points, isMajor } = calculatePointsAndMajors(place, finalEligible.length);
      if (points === 0 && place > 3) return;
      results.push({
        foxId: s.id,
        score: s.score,
        place,
