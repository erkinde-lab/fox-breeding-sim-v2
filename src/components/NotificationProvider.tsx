          ...
          {toasts.map((t) => (
            <motion.div
              ...
              className={`flex items-start gap-3 p-4 rounded-2xl border-2 shadow-lg backdrop-blur-md ${
                t.type === 'success' ? 'bg-secondary/10 border-secondary/20 text-secondary' :
                t.type === 'destructive' || t.type === 'error' ? 'bg-destructive/10 border-destructive/20 text-destructive' :
                t.type === 'warning' ? 'bg-accent/10 border-accent/20 text-accent' :
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              <div className={`p-6 flex flex-col items-center text-center gap-4 ${
                ...
                dialogParams.type === 'warning' ? 'bg-accent/5' :
                dialogParams.type === 'success' ? 'bg-primary/5' : 'bg-secondary/5'
              }`}>
                <div className={`p-4 rounded-3xl ${
                  dialogParams.type === 'error' || dialogParams.type === 'destructive' ? 'bg-destructive/10 text-destructive' :
                  dialogParams.type === 'warning' ? 'bg-accent/10 text-accent' :
                  dialogParams.type === 'success' ? 'bg-primary/10 text-primary' : 'bg-secondary/10 text-secondary'
                }`}>
                  ...
                  <>
                    ...
                    <Button
                      ...
                      className={`flex-1 h-12 rounded-xl font-black uppercase tracking-widest text-xs shadow-lg ${
                        ...
                        dialogParams.type === 'warning' ? 'bg-accent hover:bg-accent/90 shadow-accent/20' :
