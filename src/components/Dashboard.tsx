  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
        <StatCard label="Kennel Capacity" value={`${foxList.length} / ${kennelCapacity}`} icon={<Home size={24} />} variant="primary" />
        <StatCard label="Total Points" value={totalShowPoints.toLocaleString()} icon={<Trophy size={24} />} variant="secondary" />
        <StatCard label="Best in Show" value={bisWins} icon={<Star size={24} />} variant="primary" />
        <StatCard label="Best Dog" value={bestDogWins} icon={<Trophy size={24} />} variant="primary" />
        <StatCard label="Best Vixen" value={bestVixenWins} icon={<Trophy size={24} />} variant="primary" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="folk-card border-2 border-border bg-card overflow-hidden shadow-sm">
          <CardHeader className="pb-4">
            <div className="flex items-center gap-3">
              <div className="p-2.5 bg-primary/10 rounded-xl"><Heart className="text-primary fill-primary/20" size={20} /></div>
            ...
            {whelpingReports.length > 0 ? (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {whelpingReports[0].kits.length === 0 && <p className="text-xs italic text-muted-foreground">No kits were born.</p>}
                  {whelpingReports[0].kits.map((kit, i) => (
                    <div key={i} className="flex justify-between items-center p-4 bg-muted/20 hover:bg-muted/40 transition-colors rounded-2xl border border-border/50 group">
                      ...
                      <Badge variant="outline" className="font-black text-[10px] uppercase tracking-tighter border-primary/20 text-primary px-3">{kit.phenotype}</Badge>
                    </div>
                  ))}
                </div>
              </div>
            ) : <div className="text-center py-10 opacity-40"><p className="text-sm text-foreground font-medium italic">No recent whelping events.</p></div>}
          </CardContent>
        </Card>
        ...
        <Card className="folk-card border-2 border-border bg-card overflow-hidden shadow-sm">
          <CardHeader className="pb-4">
            <div className="flex items-center gap-3">
              <div className="p-2.5 bg-secondary/10 rounded-xl"><Star className="text-secondary fill-secondary/20" size={20} /></div>
          ...
          <CardContent>
                ...
                {showReports.slice(0, 1).map((report: ShowReport, idx: number) => (
                  <div key={idx} className="space-y-4">
                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/40 border-b border-border pb-2">
                      ...
                      <span className="text-secondary">{report.circuit} Show</span>
                    </div>
                    <div className="space-y-2">
                      {report.results.filter(r => r.title === "BIS" || r.title === "BOS").slice(0, 3).map((res, i) => (
                        <div key={i} className="flex justify-between items-center p-3 bg-muted/20 hover:bg-muted/40 transition-colors rounded-xl border border-border/30 group">
                          <span className="text-sm font-bold text-foreground group-hover:text-secondary transition-colors italic">{foxes[res.foxId]?.name || "Unknown"}</span>
                          <div className="flex items-center gap-3">
                            <Badge variant="secondary" className="font-black text-[10px] px-2 h-5 bg-background border-border">{res.title}</Badge>
                            <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">{res.variety}</span>
                          </div>
                        </div>
                      ))}
                      {report.bisFoxId && foxes[report.bisFoxId] && (
                        <div className="mt-4 p-4 bg-secondary/5 border-2 border-secondary/20 rounded-2xl flex justify-between items-center group hover:bg-secondary/10 transition-all">
                          <div className="flex items-center gap-3"><Trophy size={18} className="text-secondary" /><span className="font-black italic text-foreground tracking-tight">Best In Show</span></div>
                          <span className="font-black text-secondary uppercase tracking-widest text-xs group-hover:scale-110 transition-transform">{foxes[report.bisFoxId].name}</span>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            ) : <div className="text-center py-10 opacity-40"><p className="text-sm text-foreground font-medium italic">No recent show results.</p></div>}
          </CardContent>
        </Card>
      </div>
...
function StatCard({ label, value, icon, variant }: { label: string, value: string | number, icon: React.ReactNode, variant: 'primary' | 'secondary' }) {
  return (
    <div className="bg-card p-6 rounded-[32px] border-2 border-border shadow-sm flex items-center gap-5 group hover:border-primary/20 transition-all hover:translate-y-[-4px] hover:shadow-xl hover:shadow-primary/5">
      <div className={cn("p-4 rounded-[1.5rem] transition-all duration-500 group-hover:scale-110 group-hover:rotate-3 shadow-inner", variant === 'primary' ? "bg-primary/10 text-primary shadow-primary/5" : "bg-secondary/10 text-secondary shadow-secondary/5")}>{icon}</div>
      <div><p className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-[0.2em] mb-1">{label}</p><p className="text-2xl font-black italic text-foreground tracking-tight">{value}</p></div>
    </div>
  );
}
  ...
  return (
    <div className="bg-card p-5 rounded-[2.5rem] border-2 border-border shadow-sm group hover:border-secondary/30 transition-all hover:translate-y-[-2px] hover:shadow-lg">
      <div className="flex items-start gap-4">
        <div className={cn("p-3.5 rounded-2xl transition-all duration-500 group-hover:rotate-12 shadow-inner shrink-0", variant === 'primary' ? "bg-primary/10 text-primary shadow-primary/5" : "bg-secondary/10 text-secondary shadow-secondary/5")}><Icon size={22} /></div>
        <div className="min-w-0">
          <div className="flex justify-between items-start gap-2 mb-1"><h4 className="font-black italic text-foreground text-lg tracking-tight truncate group-hover:text-secondary transition-colors leading-tight">{title}</h4></div>
          <p className="text-xs text-muted-foreground font-medium mb-3 line-clamp-2 leading-relaxed">{description}</p>
          <div className="flex items-center gap-2 text-[9px] font-black uppercase text-muted-foreground/40 tracking-[0.2em]"><span>Year {year}</span><span className="w-1 h-1 rounded-full bg-border" /><span className="text-secondary/60">{season}</span></div>
        </div>
      </div>
    </div>
  );
