    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Adopt high-quality foxes with clean pedigrees. New foxes available
          hourly with guaranteed variety.
        </p>
        <div className="mt-4 p-4 bg-muted/50 rounded-lg border-l-4 border-primary/30">
          <h4 className="font-black text-sm mb-2">
            Adoption Schedule & Genetics
          </h4>
          <ul className="text-xs text-muted-foreground space-y-1">
            <li>
              • <strong>Hourly Refresh:</strong> New collection of 6 foxes every
              hour
            </li>
            <li>
              • <strong>Guaranteed Variety:</strong> At least one red base and
              one cross fox per hour
            </li>
            <li>
              • <strong>Black Fox Limit:</strong> Maximum one black fox per hour
              (high rarity)
            </li>
            <li>
              • <strong>Rare Gene Chance:</strong> Each fox has a 25% chance of
              carrying a heterozygous rare gene (Gg, Cc, Pp, Ss, Fifi, Ll)
            </li>
            <li>
              • <strong>Foundation Quality:</strong> All foxes have clean
              pedigrees for breeding programs
            </li>
          </ul>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {foundationFoxes.map((fox, index) => {
          ...
          return (
            <Card
              key={index}
              aria-label={`Adopt ${fox.phenotype} ${fox.gender}`}
              className={cn(
