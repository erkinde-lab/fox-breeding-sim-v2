...
import {
  ShoppingBag, Coins, ShoppingCart, Zap, Shield, Sparkles,
  Moon, Sun, Wind, Heart, Microscope, Pill, Calculator,
  Package, LayoutGrid, Utensils
} from 'lucide-react';
  ...
  return (
    <div className="space-y-12 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-4xl font-folksy text-foreground tracking-tight" style={{ fontWeight: 400 }}>Supplies & Feeds</h1>
          <p className="text-muted-foreground mt-2 font-medium">Essential items and nutritional supplements for your kennel.</p>
        </div>
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {SPECIALTY_ITEMS.map((item) => (
              <Card key={item.id} className="flex flex-col border-border bg-card hover:border-primary/30 transition-all overflow-hidden group rounded-[32px]">
                ...
                <CardContent className="mt-auto pt-4 p-6 border-t border-border bg-card">
                  <div className="flex justify-between items-center mb-4">
                    <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Owned: {inventory[item.id] || 0}</span>
                    <span className="font-black text-foreground flex items-center gap-1"><Coins size={14} className="text-yellow-600"/> {item.price}</span>
                  </div>
                  <Button
      ...
      {(activeTab === 'all' || activeTab === 'feeds') && (
        <section className="space-y-6">
          <div>
            <h2 className="text-2xl font-folksy text-foreground tracking-tight" style={{ fontWeight: 400 }}>Nutritional Supplements</h2>
            <p className="text-muted-foreground text-sm font-medium mt-1">Each bag provides 5 servings and a permanent +2 bonus to a specific trait.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            {SPECIALTY_FEEDS.map((feed) => (
              <Card key={feed.id} className={`border-border bg-card hover:shadow-lg transition-all group rounded-[32px] overflow-hidden`}>
                <CardHeader className="flex flex-col items-center gap-3 text-center pb-2 pt-6">
                  <div className={`p-4 bg-muted rounded-2xl group-hover:scale-110 transition-transform border border-border text-primary`}>
                    {feed.icon}
                  </div>
                  <div>
                    <CardTitle className="text-sm font-black min-h-[40px] flex items-center justify-center leading-tight text-foreground uppercase tracking-tight italic">{feed.name}</CardTitle>
                    <Badge variant="outline" className="text-[8px] font-black uppercase tracking-tighter bg-primary/5 text-primary border-primary/20">+{feed.stat}</Badge>
                  </div>
                </CardHeader>
...
interface KibbleCardProps {
  ...
  gold: number;
  buyItem: (id: string, price: number, currency: 'gold' | 'gems', count: number) => void;
}

function KibbleCard({ id, name, description, price, icon, inventory, gold, buyItem }: KibbleCardProps) {
  return (
    <Card className="flex flex-col border-border bg-card shadow-sm overflow-hidden rounded-[40px]">
      <CardHeader className="flex flex-col md:flex-row items-center gap-6 bg-muted/30 p-8">
        ...
        <div className="text-center md:text-left">
          <CardTitle className="text-2xl font-black italic text-foreground tracking-tight uppercase">{name}</CardTitle>
      ...
      <CardContent className="mt-auto p-8 border-t border-border bg-card">
        <div className="flex justify-between items-center mb-6">
          <div>
            <span className="text-[10px] font-black text-muted-foreground uppercase block tracking-widest">Inventory</span>
            <span className="text-xl font-black text-foreground">{inventory[id] || 0} Bags</span>
          </div>
          <div className="text-right">
            <span className="text-[10px] font-black text-muted-foreground uppercase block tracking-widest">Price</span>
