      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-4xl font-black text-earth-900 tracking-tight flex items-center gap-3">
             <ShoppingBag className="text-fire-600" /> Marketplace
          </h1>
          <p className="text-earth-500 mt-2">Trade foxes and items with other breeders worldwide.</p>
        </div>
        ...
        <div className="flex gap-2 w-full md:w-auto">
          ...
          <Button variant="outline" className="rounded-xl border-earth-200 bg-white">
            <SlidersHorizontal size={18} />
          </Button>
        </div>
      </div>

      <div className="flex border-b border-earth-200 gap-8">
        <button 
          onClick={() => setActiveTab('foxes')}
          className={cn(
            "pb-4 px-2 text-lg font-black transition-all border-b-2",
            activeTab === 'foxes' ? "border-fire-600 text-earth-900" : "border-transparent text-earth-400 hover:text-earth-600"
          )}
        >
          Foxes for Sale
        </button>
        <button 
          ...
          className={cn(
            "pb-4 px-2 text-lg font-black transition-all border-b-2",
            activeTab === 'items' ? "border-fire-600 text-earth-900" : "border-transparent text-earth-400 hover:text-earth-600"
          )}
        >
          Item Exchange
        </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          {otherListings.length === 0 ? (
            <div className="py-20 text-center bg-white rounded-3xl border-2 border-dashed border-earth-200">
               <ShoppingBag className="w-16 h-16 text-earth-100 mx-auto mb-4" />
               <p className="text-earth-400 font-bold">No {activeTab} currently listed by others.</p>
            </div>
          ) : (
        ...
        <div className="space-y-6">
          <h3 className="text-xl font-bold text-earth-900 flex items-center gap-2">
            <ArrowLeftRight className="text-moss-600" size={20} /> Your Listings
          </h3>
          <div className="space-y-3">
             {myListings.map((l) => (
                <div key={l.id} className="p-4 bg-white border border-earth-200 rounded-2xl flex justify-between items-center group">
                   <div>
             ...
             {myListings.length === 0 && (
                <p className="text-sm text-earth-400 italic bg-earth-50 p-4 rounded-2xl border border-dashed border-earth-200 text-center">
    ...
    return (
        <Card className="folk-card overflow-hidden">
            <div className="p-4 flex gap-4">
                <div className="w-24 h-24 bg-earth-50 rounded-2xl flex items-center justify-center flex-shrink-0 relative overflow-hidden">
                    {isFox ? (
                        <FoxIllustration phenotype={listing.foxData!.phenotype} />
                    ) : (
                        <ShoppingBag size={32} className="text-earth-200" />
                    )}
                </div>
                <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                        <h4 className="font-bold text-earth-900 truncate">
                            {isFox ? listing.foxData!.name : listing.targetId.replace(/-/g, ' ')}
                        </h4>
                        <Badge variant="outline" className="text-[9px] h-4">#{listing.id}</Badge>
                    </div>
                    <p className="text-xs text-earth-500 truncate mb-2">
            ...
            <div className="px-4 pb-4 pt-2 border-t border-earth-50 flex items-center justify-between gap-4">
                <div className={cn(
                ...
                <Button 
                    ...
                    className={cn(
                        "font-bold px-6",
                        listing.currency === 'gold' ? "bg-yellow-600 hover:bg-yellow-500" : "bg-cyan-600 hover:bg-cyan-500"
                    )}
                >
                    Buy Now
                </Button>
